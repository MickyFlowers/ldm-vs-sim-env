...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load("/home/cyx/project/latent-diffusion/logs/screwdriver-sim-m/epoch=000632.ckpt")["state_dict"],
        strict=True,
    )
    ...
    if os.path.exists("./roma_log.txt"):
        with open(f"./roma_log.txt", "r") as f:
    ...
    while simulation_app.is_running():
        ...
        if vs_env.sim_step >= 10:
            if vs_env.sim_step - 10 == 0:
                ...
                vs_mask = cv2.resize(vs_mask.astype(np.uint8), (0, 0), fx=scale, fy=scale).astype(np.bool_)
