...
class env(object):
    ...
    def reset(self, fixed_tool=False, env_ids=None):
        ...
        if fixed_tool:
            ...
            self.sample_tool_pose(env_ids=env_ids)
            self.sample_camera_to_tool(env_ids=env_ids)
        ...
        self.scene["charge"].write_root_pose_to_sim(tool_pose)
    ...
    def sample_camera_to_tool(self, env_ids=None):
        ...
        default_camera_pose[:, :3, 3] = torch.tensor([0.0, 0.18, -0.07])
        default_camera_pose[:, :3, :3] = math_utils.matrix_from_euler(
            torch.tensor([torch.pi / 2, -torch.pi / 180.0 * 10, torch.pi / 180.0 * 2]),
            "XYZ",
        )
    ...
    def sample_tool_pose(self, env_ids=None):
        if env_ids is None:
            env_ids = [i for i in range(self.scene.num_envs)]
        sample_trans_matrix = sample_coordinate_in_cardisian(
            self.scene.num_envs,
        ...
        default_tool_pose_mtx = (
            torch.eye(4).repeat(self.scene.num_envs, 1, 1).to(self.scene.device)
        )
        self.default_tool_pose_rot_mtx = math_utils.matrix_from_quat(
            self.tool_default_root_state[:, 3:]
        )
        default_tool_pose_mtx[:, :3, :3] = self.default_tool_pose_rot_mtx
        default_tool_pose_mtx[:, :3, 3] = self.tool_default_root_state[:, :3]
        self.tool_pose_trans_matrix[env_ids, ...] = (
            default_tool_pose_mtx[env_ids, ...] @ sample_trans_matrix[env_ids, ...]
        )
    ...
    def sample_camera_world_pose(self, env_ids=None):
        ...
        sample_camera_world_pose_trans_matrix = sample_coordinate_in_sphere(
            self.scene.num_envs,
            radius_range=[0.4, 0.5],
            u_range=[0.0, torch.pi / 4],
            v_range=[-16 * torch.pi / 30, -14 * torch.pi / 30],
        ).to(self.scene.device)
    ...
    def get_observation(self):
        ...
        mask = (
            (
                ...
                == torch.tensor(
                    str_to_color_tuple(class_to_color_dict["charge"]),
                    device=self.scene.device,
                )
            ).all(dim=-1)
            if "charge" in class_to_color_dict
            else torch.full(
        ...
        switch_mask = (
            (
                camera_data["semantic_segmentation"]
                == torch.tensor(
                    str_to_color_tuple(class_to_color_dict["charge2"]),
                    device=self.scene.device,
                )
            ).all(dim=-1)
            if "charge2" in class_to_color_dict
            else torch.full(
                camera_data["semantic_segmentation"].shape[:3],
                False,
                device=self.scene.device,
