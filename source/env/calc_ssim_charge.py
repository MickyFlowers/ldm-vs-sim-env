...
if __name__ == "__main__":
    ...
    # ibvs_controller = IBVS(camera=camera, kp_algo=RomaMatchAlgo)
    config = OmegaConf.load(
        "/home/cyx/project/latent-diffusion/logs/charge-sim-m/2025-09-24T17-46-16-project.yaml"
    )
    model = instantiate_from_config(config.model)
    model.load_state_dict(
        torch.load(
            "/home/cyx/project/latent-diffusion/logs/charge-sim-m/epoch=000089.ckpt"
        )["state_dict"],
        strict=True,
    )
    device = (
    ...
    while simulation_app.is_running():
        ...
        if vs_env.sim_step >= 10:
            if vs_env.sim_step - 10 == 0:
                ...
                if debug:
                    cv2.imwrite("ref_color_image.png", ref_color_image)
                ...
                with torch.no_grad():
                    with model.ema_scope():
                        ...
                        # )
                        if debug:
                            cv2.imwrite("generated.png", reference_image)
                            generated_grey = cv2.cvtColor(
                                reference_image, cv2.COLOR_BGR2GRAY
                            )
                            gt_grey = cv2.cvtColor(gt, cv2.COLOR_BGR2GRAY)
                            error_image = cv2.absdiff(generated_grey, gt_grey)
                            cv2.imwrite("error.png", error_image)
                            ssim = calc_ssim(reference_image, gt)
                            vs_env.sim_step = 10
                            total_ssim += ssim
