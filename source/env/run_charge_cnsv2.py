...
if __name__ == "__main__":
    ...
    config = OmegaConf.load("/home/cyx/project/latent-diffusion/logs/charge-sim-m/2025-09-24T17-46-16-project.yaml")
    model = instantiate_from_config(config.model)
    model.load_state_dict(
        torch.load("/home/cyx/project/latent-diffusion/logs/charge-sim-m/epoch=000089.ckpt")["state_dict"],
        strict=True,
    )
    ...
    if os.path.exists("./charge_log.txt"):
        with open(f"./charge_log.txt", "r") as f:
    ...
    while simulation_app.is_running():
        ...
        if vs_env.sim_step >= 10:
            if vs_env.sim_step - 10 == 0:
                ...
                vs_env.sample_camera_world_pose()
                vs_env.step()
                obs = vs_env.get_observation()
                ...
                _, vel, _, match_img = ibvs_controller.calc_vel(
                    depth_hint=np.mean(ref_depth),
                )
                ...
                if score is not None and score > 0.90:
                    ...
                    if count > 60:
                        ...
                        with open(f"./charge_log.txt", "a") as f:
                            f.write(
